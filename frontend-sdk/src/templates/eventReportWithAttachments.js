...
const eventReportWithAttachments = async (reportData, clubName, view = false) => {
  const multiplePdfs = [];
  ...
  const reportDetails = async () => {
    var opt = {
      margin: 0.5,
      filename: `Event-Report-${reportData?.eventName}.pdf`,
    ...
    await html2pdf()
      .from(getEventReport(reportData, clubName))
      .set(opt)
      .toPdf()
      .output("datauristring")
      .then(async function (pdfAsString) {
  ...
  async function eventImages() {
    if (reportData?.images != null) {
      for (var i = 0; i < reportData.images.length; i++) {
        await imageOrPdf(reportData.images[i]);
      }
    }
  }
  ...
  async function mergePdfs() {
    ...
    if (view) {
      ...
      link.download = `Event Report - ${reportData?.eventName}.pdf`;
...
export default eventReportWithAttachments;