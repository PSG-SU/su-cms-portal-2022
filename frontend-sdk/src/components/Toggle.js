...
const ToggleItem = ({
  ...
  return (
    <button
      ...
      className={`${className} h-full relative  border-yellow ${
        isSelected && "bg-[#F1E6C9]"
      } border-2 rounded-lg p-4 px-6 space-x-2 flex-1 flex flex-row items-center text-left`}
    >
      ...
      <p>
        {icon &&
          React.cloneElement(icon, {
            className:
              "hidden lg:block text-yellow absolute top-[50%] right-2 -translate-y-[50%] opacity-[50%]",
            size: 64,
          })}
      </p>
    </button>
