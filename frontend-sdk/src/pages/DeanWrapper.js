...
import DeanSUMenuItems from "./Dean.routes.js";
...
const DeanWrapper = () => {
  ...
  useEffect(() => {
    if (
      ...
      localStorage.getItem("rights") === "dean"
    ) {
      axios
        .get(LOGIN_URL, {
          headers: {
  ...
  return (
    <main className="h-screen w-screen overflow-auto flex font-sans">
      <nav className="bg-gradient-to-bl from-blue to-black shadow-lg w-1/4 overflow-hidden py-8 h-screen">
        ...
        <p className="text-xs uppercase tracking-wider mb-8 px-8 text-white">Dean Panel</p>
